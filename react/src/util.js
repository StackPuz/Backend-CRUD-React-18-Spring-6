...
export default {
    ...
    document.querySelectorAll('input[data-type]').forEach(e => {
      e.addEventListener('keyup', () => {
        if (!e.value) {
          ...
          setValue.call(e, '')
          e.dispatchEvent(new Event('update', { bubbles: true }))
        }
      })
    })
  ...
  getLink(paging, type, value, sort = '') {
    let query = this.getQuery()
    let link = ''
    if (type == 'sort') {
  ...
  getFormData(data) {
    return Object.keys(data).reduce((form, key) => {
      if (data[key] !== null && data[key] !== undefined) {
        if (typeof data[key] == 'object' && !Array.isArray(data[key]) && !(data[key] instanceof File)) {
          Object.keys(data[key]).forEach(k => {
            form.append(`${key}.${k}`, data[key][k])
          })
        }
        else {
