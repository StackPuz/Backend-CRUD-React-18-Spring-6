            ...
            <table className="table table-sm table-striped table-hover">
              <thead>
                <tr>
                  ...
                  <th className={Util.getSortClass('image') + ' d-none d-md-table-cell' }>
                    <Link to={Util.getLink(paging,'sort','image')}>Image</Link>
                  </th>
                  <th className={Util.getSortClass('name')}>
                    <Link to={Util.getLink(paging,'sort','name')}>Name</Link>
                  </th>
                  <th className={Util.getSortClass('price') + ' d-none d-md-table-cell' }>
                    <Link to={Util.getLink(paging,'sort','price')}>Price</Link>
                  </th>
              ...
              <tbody>
                ...
                <tr key={index}>
                  ...
                  <td className="d-none d-md-table-cell text-center">{ product.image && <a href={`http://localhost:8080/app/uploads/products/${product.image}`} target="_blank" rel="noreferrer" title={`${product.image}`}><img className="img-list" src={`http://localhost:8080/app/uploads/products/${product.image}`} /></a> }</td>
                  <td>{product.name}</td>
                  <td className="d-none d-md-table-cell text-right">{product.price}</td>
                  <td className="d-none d-md-table-cell">{product.brand && product.brand.name}</td>
                  <td className="d-none d-md-table-cell">{product.userAccount && product.userAccount.name}</td>
