          ...
          <form method="post" onSubmit={remove}>
            <div className="row">
              ...
              <div className="form-group col-md-6 col-lg-4">
                ...
                <input readOnly id="brand_name" name="brand_name" className="form-control form-control-sm" value={(product.brand && product.brand.name) || '' } maxLength="50" />
              </div>
              <div className="form-group col-md-6 col-lg-4">
                <label htmlFor="user_account_name">Create User</label>
                <input readOnly id="user_account_name" name="user_account_name" className="form-control form-control-sm" value={(product.userAccount && product.userAccount.name) || '' } maxLength="50" />
              </div>
              <div className="form-group col-md-6 col-lg-4"><label>Image</label>
                <div><a href={`http://localhost:8080/app/uploads/products/${product.image || '' }`} target="_blank" rel="noreferrer" title={`${product.image || '' }`}><img className="img-item" src={`http://localhost:8080/app/uploads/products/${product.image || '' }`} /></a></div>
              </div>
