          ...
          <form method="post" onSubmit={edit}>
            <div className="row">
              <div className="form-group col-md-6 col-lg-4">
                ...
                <input readOnly id="order_detail_order_id" name="id.orderId" className="form-control form-control-sm" onChange={onChange} value={(orderDetail.id && orderDetail.id.orderId) || '' } type="number" required />
                {errors.id && <span className="text-danger">{errors.id}</span>}
              </div>
              <div className="form-group col-md-6 col-lg-4">
                <label htmlFor="order_detail_no">No</label>
                <input readOnly id="order_detail_no" name="id.no" className="form-control form-control-sm" onChange={onChange} value={(orderDetail.id && orderDetail.id.no) || '' } type="number" required />
                {errors.id && <span className="text-danger">{errors.id}</span>}
              </div>
              <div className="form-group col-md-6 col-lg-4">
                <label htmlFor="order_detail_product_id">Product</label>
                <select id="order_detail_product_id" name="product.id" className="form-control form-control-sm" onChange={onChange} value={(orderDetail.product && orderDetail.product.id) || '' } required>
                  <option></option>
